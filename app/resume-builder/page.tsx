...
const ResumeBuilder = () => {
  ...
  const generateResume = async () => {
    ...
    experiences.forEach(exp => {
      page.drawText(exp.company, { x: 50, y: currentY, size: 11, font: boldFont });
      page.drawText(exp.years, { x: 450, y: currentY, size: 9, font });
      currentY -= 15;
      page.drawText(exp.role, { x: 50, y: currentY, size: 10, font, color: rgb(0.3, 0.3, 0.3) });
      currentY -= 25;
    });
    ...
    education.forEach(ed => {
      page.drawText(ed.school, { x: 50, y: currentY, size: 11, font: boldFont });
      page.drawText(ed.years, { x: 450, y: currentY, size: 9, font });
      currentY -= 15;
      page.drawText(ed.degree, { x: 50, y: currentY, size: 10, font });
      currentY -= 25;
    });
          ...
          <div className="flex-1 space-y-8 max-h-[85vh] overflow-y-auto pr-4 custom-scrollbar">
            ...
            <div className="p-8 rounded-4xl bg-white/5 border border-white/10 space-y-6">
              ...
              <div className="space-y-2">
                ...
                <textarea 
                   className="w-full bg-white/5 border border-white/10 rounded-xl p-4 min-h-25 outline-none focus:border-purple-500 transition-all"
            ...
            <DynamicSection 
              ...
              renderInputs={(item: any) => (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <input placeholder="Company" className="form-input" value={item.company} onChange={(e) => setExperiences(experiences.map(x => x.id === item.id ? {...x, company: e.target.value} : x))} />
                  <input placeholder="Role" className="form-input" value={item.role} onChange={(e) => setExperiences(experiences.map(x => x.id === item.id ? {...x, role: e.target.value} : x))} />
                  <input placeholder="Years" className="form-input" value={item.years} onChange={(e) => setExperiences(experiences.map(x => x.id === item.id ? {...x, years: e.target.value} : x))} />
                </div>
              )}
            />
            ...
            <DynamicSection 
              ...
              renderInputs={(item: any) => (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <input placeholder="School/Uni" className="form-input" value={item.school} onChange={(e) => setEducation(education.map(x => x.id === item.id ? {...x, school: e.target.value} : x))} />
                  <input placeholder="Degree" className="form-input" value={item.degree} onChange={(e) => setEducation(education.map(x => x.id === item.id ? {...x, degree: e.target.value} : x))} />
                  <input placeholder="Years" className="form-input" value={item.years} onChange={(e) => setEducation(education.map(x => x.id === item.id ? {...x, years: e.target.value} : x))} />
                </div>
              )}
            />

            <button onClick={generateResume} className="w-full py-5 bg-purple-600 rounded-3xl font-black text-xl hover:bg-purple-500 transition-all flex items-center justify-center gap-3">
                 ...
                 <section>
                   ...
                   {experiences.map(exp => (
                     <div key={exp.id} className="mb-2">
                       <div className="flex justify-between font-bold"><span>{exp.company}</span><span>{exp.years}</span></div>
                 ...
                 <section>
                   ...
                   {education.map(ed => (
                     <div key={ed.id} className="mb-2">
                       <div className="flex justify-between font-bold"><span>{ed.school}</span><span>{ed.years}</span></div>
...
const DynamicSection = ({ title, icon, items, onAdd, onRemove, renderInputs }: any) => (
  <div className="p-8 rounded-4xl bg-white/5 border border-white/10 space-y-6">
    ...
    {items.map((item: any) => (
      <div key={item.id} className="relative p-6 bg-white/5 rounded-2xl border border-white/5 group">
        <button onClick={() => onRemove(item.id)} className="absolute -top-2 -right-2 p-2 bg-red-500/10 text-red-500 rounded-full opacity-0 group-hover:opacity-100 transition-all hover:bg-red-500 hover:text-white"><Trash2 size={14} /></button>
        {renderInputs(item)}
      </div>
    ))}
  </div>
